 ...
 */
export function parse_participant(
  participant: any // eslint-disable-line @typescript-eslint/no-explicit-any
): VtbParticipant {
 ...
 */
export function parse_extra_field(
...
export function parse_element_unit(
  element_data: any, // eslint-disable-line @typescript-eslint/no-explicit-any
  vtb_element: VtbElement,
  vtb_element_price: number
): VtbElementUnit {
  // setup base element
  const vtb_element_unit = new VtbElementUnit();
  // console.debug('element_data: ', element_data);

  vtb_element_unit.title = element_data.title;
  vtb_element_unit.subtitle = element_data.subtitle;
  ...
  // vtb_element_unit.description = element_data.description;
  ...
  vtb_element_unit.additional_description = element_data.subAdditionalText
    ? element_data.subAdditionalText?.replace(re_body, '$1')?.replace(re_style, '')
    : '';
...
export function parse_element(
  ...
  const vtb_element_unit = parse_element_unit(
    element_data,
    vtb_element,
    vtb_element_price
  );
 ...
 */
export function parse_marker(
  marker_data: any, // eslint-disable-line @typescript-eslint/no-explicit-any
  title?: string
): VtbMapMarker {
...
export function parse_segment(
  ...
  for (const element_data of segment_data.elements) {
    const vtb_element: VtbElement = parse_element(
      element_data,
      segment_data.title
    );
...
export class VtbDataTransformer {
  ...
  parse_vtb_data(
    ...
    for (const segment_data of vtbSrcData.segments) {
      ...
      const vtb_element_group = parse_segment(
        segment_data,
  ...
  protected parse_carrental_elements(
    ...
    for (const carElementData of segment_data) {
      const car_element = parse_element(
